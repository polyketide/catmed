...
4. **⭐ Helping the owner carry the one thing only they have: the time series.** The clinician sees snapshots at appointments; the owner sees every day in between. That longitudinal record is not a nice-to-have — it is **structurally unobtainable by anyone else in the chain**, and it is where decoupling (energy recovers, appetite does not), lag (a drug effect appearing days later) and trend-versus-noise actually become visible. When an owner is tracking, help them track in a way that survives contact with a busy specialist:
   - **Anchor every subjective scale in words** (`0 = 完全不吃`, `5 = 主动吃但明显少于平常`, `10 = 与患病前无异`). An unanchored 1–10 drifts across weeks and across family members, which quietly destroys the data's value.
   - **Track the outcome, not only the input.** Calories are the input; **body weight is the outcome.** "Calories look adequate and weight is still falling" is the signal worth catching, and input-only tracking cannot see it.
   - **Plot interventions on the same axis as the observations** — doses, drug holidays, new medications. Lags and decoupling are invisible on separate charts.
   - **Triage the questions**: for each one, ask *"what would we do differently depending on the answer?"* A question that changes no action is expensive to put to someone with no time.
   - **Compress the presentation, never the observation.** Record in detail; hand over one page. If a clinician cannot find the single thing needing their judgement within about 30 seconds, it is too long — and length consumes exactly the resource it was meant to save.
...
5. **⭐ Preparing an owner for a specialist consultation — a question list has a lifecycle, and it decays.** When an owner is heading to a referral or second opinion, producing "a list of good questions" is not the job. Work these, in this order:
   - **Two documents, two recipients — never translate one into the other.** The owner's working copy carries the ordering rationale, the do-not-say list, and the premises they have not verified. The clinician's copy carries the patient, the observations only the owner has, the trends, and the questions — in the *clinician's* working language, compressed. Handing over the working copy exports your reasoning process as if it were information; producing only the clinician copy leaves the owner without navigation under pressure.
   - **Separate materials from questions.** If the answer is not in the hands of the clinician they are about to see, it is not a question — it is a **material to obtain beforehand, usually from a different party**: the written drug history, a report already issued but never received, a referral letter carrying the specimen accession number. Miss this and the highest-value, lowest-risk asks — "can you add a stain to the existing slide" — collapse on the day, because nobody can retrieve the specimen.
   - **⭐ Check the recipient has not changed.** A list written weeks ago was addressed to whoever was in front of them then. A specialist frequently *cannot* answer it — they did not prepare the slide, they do not hold the prescribing record. The fix is not to delete those questions but to rewrite them from *"tell me X"* into *"can you retrieve X / can you add X."* Same fact, different verb: one returns "I don't know", the other starts a process.
   - **Order for interruption, not by topic.** Being called away mid-consultation is normal. Draw an explicit line (say, after the fifth question) above which the core value of the visit has already been captured. **The first question is usually not the most important one — it is the one most easily destroyed by the owner speaking first.** Open-ended, then stop talking: an independent judgement is the entire point of a second opinion, and stating your own view first contaminates it.
   - **Audit the list against the timeline before every visit.** Ask of each thing that has happened since it was written: is there a question for it? Two failure shapes recur — a test whose *methodology* was never asked about (the same "negative" can differ several-fold in exclusion power by method), and, worse, **a list still built on a strategic premise that has since changed** (e.g. imaging follow-up was abandoned on risk grounds, so "what do we judge response by now" became the central question — and the old list has no slot for it, because it did not need one when written).
   - **Write a do-not-say list too.** It prevents the misstatements that actually move a clinician's judgement: presenting "most consistent with" as confirmed; letting another animal's history slide into your own narrative; reporting co-occurrence as causation when a second drug was running; putting the derailing topic (equipment, cost, transfer) early enough to consume the visit.
   - **"Not yet received" is not "no result."** A blank cell reads as *nothing to report*; its true meaning is *I never got it*. Write **not obtained** explicitly and move it onto the materials list.
   - Full version: `knowledge-base/working-with-a-specialist.zh.md` §3b. ⚠️ Same grading — **contributed practice, not evidence.**
