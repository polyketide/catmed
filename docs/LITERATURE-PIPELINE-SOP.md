...
## 3j. ⚠️ Negative claims had no evidence at all — and the report that fixed it is deliberately allowed to be wrong

Everything in this pipeline was built to discipline **positives**. Every excerpt is byte-compared against an archived record and Leg 1 fails if one drifts. **Negatives were never checked by anything.** A file saying *"no feline study addressing this was located"* is a research claim, frequently a load-bearing one — it is the whole difference between *"we chose not to state a dose"* and *"no dose exists"* — and it rested on nothing a reader could audit.

**Measured 2026-07-27: 16 such claims across the knowledge base, none verifiable.** The archive held 285 records against 283 cited PMIDs, which says something sharper than the count does: **the archive contains essentially only what got cited.** Papers read and rejected during a search left no trace whatsoever, so the same ground gets re-searched and nobody can audit the judgement that discarded them. `docs/search-log.jsonl` and `tools/search_log.py` exist for that: query, date, database, hit count, what was kept, and **for every rejection a `why`** — a PMID with no reason records that you looked, not what you concluded. Schema and archive consistency are gated by hygiene check `search-log`; the `negatives` report is not gated, for the reason below.

**⭐ The refusal that gave it teeth on day one.** Asked to backfill the 8 still-unbacked claims, the correct move was to decline: those searches ran in earlier sessions, so writing queries and rejection reasons for them would be **inventing a search record** — §12's failure exactly, this time frozen into a tool artifact where it would read as machine-generated fact. They were **re-run instead** and logged `contemporaneous`. Two came back 0 hits and were confirmed. **One was refuted, and it was this project's own claim, written that same day: "no veterinary literature on owner–specialist communication was located."** It had never been searched — it was asserted. The search returns 25 papers, three directly relevant, and one of them (Ito 2022) *strengthened* the very file that had dismissed the field. **The log caught it within an hour of existing.** The error notice stays in the file; it is not edited away.

**⭐ And then the report flagged a false positive — which is being kept.** `negatives` matches `README.zh.md`'s line *"是本项目已经检索过、且确认没有找到的地方"*. That sentence **describes the practice**; it asserts no result. Nothing backs it because there is nothing to back. Score: 17 of 18. **No suppression was added.**

**Rules.**
1. **A statement that a search found nothing is a research claim and needs a record, exactly as a quoted finding does.** The pipeline's asymmetry — rigorous on what was found, silent on what was not — is what let an unsearched assertion sit in a knowledge-base file.
2. **⛔ Never reconstruct a search you did not perform. Re-run it, or leave the claim unbacked.** An unbacked claim is honest and visible; a fabricated record is neither, and it is worse than the gap it fills because tooling launders it. `reconstructed` exists as a grade so after-the-fact entries can never masquerade as `contemporaneous` (cf. §10: judgement recorded, never promoted).
3. **⭐ An advisory report may be wrong in the safe direction — do not suppress its false positives.** Gating checks (Leg 1, hygiene) must be exact, because they block a commit. `negatives` blocks nothing, so a false positive costs one line of reading, while a suppression list costs the reader's belief that the report is unfiltered. **This inverts §3d's lesson rather than contradicting it**: there, a suppression hatch that did not work was worse than none; here, one that *did* work would be. Match the mechanism to whether it gates.
4. **Name what is still outside — the matching is a heuristic.** `negatives` pairs prose to log entries by keyword overlap. **A ✅ means a plausibly related search was logged, never that the claim is proven**, and the judgement stays with the reader by design. The tool also does not write itself: the reason a paper was rejected is a judgement only the agent that read the abstract can supply, and an auto-logger would capture the queries and lose the only part that matters.

## 3k. ⚠️ A resolution note is not a flag — the full-text work list re-reported nine of ten already-closed gaps

`fetch_fulltext.py --needed` builds its work list from the ⚠️ flags that mark a figure as absent from the abstract. **Measured 2026-07-27: the list held 10 PMIDs, and 9 of them had already been resolved** — several of them a week earlier, in notes sitting *directly beneath the flag they closed*, inside the same excerpt block. The tenth needed no full text either.

**Why the detector could not tell.** A note that closes a flag has to say which figure it closed, so it quotes the same figures in the same backticks as the flag itself: *"The figures `50, 87` were flagged here…"*. The matcher looked for the figures, not for the request — **so the detector was matching text it had itself caused to be written.** The fix keys on the open-flag template's own wording (`retrieve the full text and verify before citing`), which a resolution never carries.

**⭐ And the deeper finding: there was no gap in the first place, in any of the ten.** Every flagged token was one of four things — a percentage *this repository computed* (`27` is arithmetic on the source's `7/26`), a citation year (`1996`, `2019`, `2024`), a PMID caught by a numeric matcher (`8947875`, `26834270`), or **another paper's number, captured by sentence-level attribution when two works are cited in one sentence**. That last is the recurring pattern this repository has now recorded **14 times** across three files.

**What was genuinely open, and it was not a full text.** `197` had been correctly reattributed to Blake 2016 — but Blake 2016 **had no excerpt block anywhere in the repository**, so the number lived only in prose. The gap had been recorded as *moved* and then never landed. It closed from the archived abstract in one lookup: *"A total of 197 doses were administered to the cats, with 33 of 68 cats receiving multiple doses, and no cats experiencing an HSR."* ⚠️ And reading it properly added a caveat the body had not carried: **0/68 cats is an absence of events, not a measured rate**, and supports *"uncommon"* rather than *"does not happen"*.

**Rules.**
1. **A detector must not match text it caused to be written.** Where a template's vocabulary is reused by the note that closes it, key the detector on the part only the open form has.
2. **⛔ Resolve a flag by editing the flag, not by writing a note beside it.** Two contradictory statements in one block means tooling reads whichever it hits first — and a stale open flag outranks a true resolution, because it is the one shaped like work. Both surviving flags were edited out rather than filtered around; a regression test asserts the filter does *not* rescue a stale flag left in place, so the corpus stays the source of truth.
...
5. **State what is still unreachable.** After a second retrieval round, **3 papers** carrying full-text excerpts still have no held PDF (two closed, one bronze behind a bot check), so Leg 5 cannot see them and says so in its own verdict line (cf. §3i rule 3: a check should name the gap it did *not* close). ⛔ The three were not obtained, and no access control was circumvented to try.
7. **⭐ Expect the extractor to be the failure, and do not let that soften the check.** Retrieving 10 more PDFs took Leg 5 from 23 to 76 excerpts and produced 21 mismatches across two runs: **19 were extraction artefacts, 2 were genuine transcription errors** — a dropped word, and a dropped `p` twice (`(< 0.0001)` where the source writes `(p < 0.0001)`, sitting next to an AUC where a bare inequality invites being read as a confidence bound). **Both were invisible to every abstract-level check, because neither sentence is in the abstract.** The artefact taxonomy and the handling of each class live in `LITERATURE-READING-SOP.md` §7.
8. **⛔ Some artefacts must be named, not tolerated.** Inline superscript reference numbers extract as text (`hypertensive cats,7,20,57,66,67 including…`), and a matcher tolerance for them would mean ignoring digits inside a sentence — in a corpus whose premise is that the digits are right. Those are recorded per excerpt in `kb-exceptions.md` under `## leg5`, **keyed on PMID plus an opening prefix** so one exception can never cover a second excerpt of the same paper, counted in their own bucket, and Leg 5 reports any exception that has gone stale — a dead hatch pointing the other way is still a dead hatch (§3d).
9. **The reading rules themselves now have their own document.** `LITERATURE-READING-SOP.md` is the veterinary adaptation of the sibling project's reading SOP: what carried over (text-first, scanned-is-not-empty, flag-not-invent, provenance), what could not (there is no local OCR here, and no digest is admissible), and **what only this project needs — species, N and setting, design and date, funding — fixed before a single result is read.**
...
### ⭐ Corpus-driven gap analysis — let a real community set the research order

Until 2026-07-26, what went into the knowledge base next was chosen by the maintainer's sense of what mattered. That sense was **measurably wrong about the ordering**, and a real corpus said so.

**The method.** A cat-cancer owner group's chat export (8045 messages, 493 members whose display names are structured case cards: age, city, tumour type, protocol and cycle count) was parsed locally, classified by topic, and set against the knowledge base's actual coverage. It took one session and produced a research queue no amount of reflection had.

**What it found, and why it was not guessable.** The single most-discussed supportive-care intervention was **G-CSF (升白针)** — 65 mentions, ahead of antiemetics and appetite stimulants — and coverage was **zero**. It was not a small gap: repeated *human* G-CSF in cats raises neutralizing antibodies that cross-react with the cat's own G-CSF, so the drug given for neutropenia can cause it, and owners in the corpus were already reporting *"some cats stop responding"* without knowing the mechanism. **A hazard being lived through, invisible to the maintainer, one grep away from being addressed.**

**And the mirror-image finding: the ordering was inverted.** Prognosis and survival — the material the knowledge base covered *best* — was **1.9%** of questions. Chemotherapy logistics, hospital choice, drug access and "can she have this cycle" dominated. **The project had built decision support for people who mostly needed execution support.** That is not a defect the corpus created; it is one the corpus made visible.

**Rules.**
1. **Verify the gap before filling it — with a grep, not an impression.** A second candidate topic (transfusion vs ESA) looked like a gap and was **not**: the reticulocyte-based fork was already covered in full. The correct output there was a **cross-reference between two files that did not point at each other**, not a new entry. **Inventing a gap to justify work is the same failure as inventing a finding.**
2. **The corpus sets priority; it never supplies evidence.** Nothing from a chat may be cited, quoted, or entered as a source — it is not a §7a source in any form, and the excerpt discipline is unchanged. Every claim that lands still comes from PubMed, verbatim, archived. A community tells you **what to go and check**, never **what is true**.
3. **Private corpora stay private, including their aggregates.** Chat text, display names, cat names and cities never enter the repository. Even *de-identified aggregate statistics* about a private group are derived from data nobody consented to publish — those belong in a private artifact, not a public repo. What may cross into a public entry is the medical literature the corpus pointed at, plus non-identifying generalities ("owner communities report X"). **This repository's own privacy screen does not catch this class**, because the risk is not a named entity — it is a whole dataset.
4. **Ask the community for its own semantics; do not decode them.** 31 members carried star markers (⭐🌟✨). Their meaning was genuinely ambiguous from the data, so they were **flagged as uncertain and excluded**, and the deceased count reported as 15 (3.0%). A human who knows the community then confirmed that a star means **去了喵星** — deceased. Corrected, the count is **46 (9.3%), triple the original**, and *still* an undercount, because owners who lose a cat usually leave the group. **The right move was to flag rather than guess** (§10's judgement grade, applied to community vernacular) — a guess either way would have been wrong by 3×, and the flag is what made the correction possible.

### ⭐ Question-driven research — one owner's question, answered properly, is a second research instrument

The corpus method above ranks topics by **frequency**. It is blind to the topic asked *once*, by one person, thinking carefully. On 2026-07-27 two such questions arrived from the same owner, and each opened an area where knowledge-base coverage measured **zero**:

- *"My cat's ocular lesion grew slightly a week after the first vincristine dose and the drug was written off — can a small early change really establish non-response?"* → `RECIST`, `response criteria`, `cross-resistance`, `P-glycoprotein`, `intraocular` — **all nil** before this. Landed as `response-assessment-and-drug-resistance.md`.
- *"Owners discuss whether the tumour shrinks; nobody discusses what the next drug risks. Many cats die from the treatment, not the tumour."* → `treatment-related death`, `toxic death`, `therapeutic index` — **all nil**. Landed as `treatment-related-harm-and-the-tradeoff.md`.

**Neither would have surfaced from a frequency count.** They are the long tail, and the long tail is where the unexamined assumptions live. **Frequency finds what many people need; a good question finds what nobody has checked.** Run both.

**Rules.**
1. **Steelman first: find the strongest version of the argument, which is often not the one offered.** The owner argued *cell kinetics* — growth outpacing kill can produce net enlargement despite real cytotoxicity. That is sound arithmetic but **no feline study measuring it was located**, so it stays labelled as reasoning. The far stronger support was a **measurement-validity** point they had not made: the VCOG response criteria state in their own scope that they are *"intended only for use in dogs"* and *"do not critically assess extranodal disease"* — and an intraocular lesion is extranodal. **The job is to find the best argument for the question, not to grade the version supplied.**
2. **Look for the confounder the asker did not name.** They did not mention **uveitis**, which accompanies 75% of feline intraocular lymphoma and moves in the *same direction* as progression — so the eye is a readout whose noise mimics the signal being excluded. The most valuable contribution was a variable absent from the question.
3. **"No evidence" is a finding, and is sometimes the whole finding.** For treatment-related mortality the answer was not a rate — it was that the largest feline CHOP series **explicitly could not report toxicity**, because in gastrointestinal disease the illness and the drug produce the same signs. Quoting a paper's own limitations verbatim is stronger than any number this project could have assembled, and it is the honest answer to *"how many cats die from treatment?"*: **not well established, for a structural reason.**
4. **Answer both directions of the bias, especially when one direction flatters the asker.** The owner's perception may be right *because studies undercount harm*; it may also be inflated *because a support group amplifies dramatic events and never discusses the cat that quietly tolerated twelve cycles*. **Both go in the file.** Confirming a sympathetic hypothesis while omitting its counter-bias is the same failure as §12's confident negative, pointed the other way.
